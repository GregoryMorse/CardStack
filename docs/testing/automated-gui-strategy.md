...
- Dialog geometry smoke tests: decoded Win16 dialog units produce stable Qt positions and sizes, with tolerance for font and platform differences; visible controls must stay within their dialog and avoid material sibling overlap except for expected group-box/frame containment. Populated comboboxes are checked separately for popup width, visible-row capacity, and style-computed closed text area with the dropdown arrow reserved. Find/Replace also has an explicit minimum-clearance regression because non-overlapping rectangles alone do not guarantee comfortable spacing.
- Live toolbar gallery: raw toolbar/control-strip resources are not treated as modal dialogs. `MainWindowActionTests` captures the real startup, deck, unselected designer, and selected template Text/Data/Notes/Line-or-Box and report Text/Data/System/Line-or-Box states.

## Residual behavioral test gaps

- Selection-specific property bands are visually captured for every supported frame kind. Message-driven package tests now edit template field Name/Length/Phone/Show name and report Shape/Line style/Fill pattern/Corner radius through the live toolbar, export SQLite packages, reload them, and compare persisted values. Remaining depth is combinatorial interaction coverage rather than an untested persistence path.
- Menu and toolbar mode switching now includes an automated deck-to-report MDI activation matrix that checks command-set replacement after each focus transfer. Manual MDI evidence remains for platform window-manager behavior and larger activation permutations.
- Gallery and geometry tests detect clipping, overlap, closed-combo truncation, and selected-control toolbar composition. They do not establish pixel identity with Win16; display-scale, native font, keyboard traversal, high-contrast, and translated-string expansion remain release evidence.
