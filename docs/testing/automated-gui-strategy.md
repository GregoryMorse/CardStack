...
- Covered by `MainWindowWorkflowTests`: Close All prompt sequencing, app Exit prompt sequencing, Available Reports opening on empty report lists, Add Defaults, Delete/Undo Del, report manager New/Modify, report designer text/data/system/line tool routing and save, modern security add/remove with encrypted flag and wrong-password retry, Find/Replace/Replace All/Index dialog workflows, design-from-scratch close/use-template flow, pattern-after-deck-file flow, phone quick-dial configuration feeding Phone > Dial, and current-card phone numbers with configured dialing prefixes.
- Covered by widget/core/storage/legacy suites: blank editable card behavior, last-card delete behavior, undelete, title sort refresh, deck merge, template definitions, decoded dialog/resource construction, no-overlap/out-of-bounds dialog geometry smoke checks, combobox popup width/row geometry smoke checks, modernized phone/print/new-dialog sizing regressions, report pagination/rendering, deterministic PDF rendering through `QPdfWriter`, SQLite deck/package persistence, and WineVDM fixture import smoke checks when fixture environment variables are present.
